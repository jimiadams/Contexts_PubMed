##############################################################
# Here we address the notorious problem of name disambiguation.
# This script matches authors by considering common last names, 
# university affiliations, and emails.
# 
# It builds off off the pubmed_articles2.Rda and pubmed_authors files from step 1
#
# This is built from an internal function (authors_clean) from 
# refsplitr (https://github.com/ropensci/refsplitr)
# 
#Note that depending on the size of the pull from pubmed this can take hours to run.
###############################################################
...
nauth <- length(unique(pma$pmid))

pma <- pma2 %>% add_count(pmid)

pma <- pma %>% filter(n>1)


pma$id <- as.numeric(row.names(pma))

pma$first <- trimws(tolower(pma$firstname))

pma$middle <- sub(" .*", "", pma$first)

pma$middle <- stringr::word(pma$first, 2)

pma$last <- trimws(tolower(pma$lastname))

pma <- pma %>% select(first, middle, last, address, email, id)



  # Get the data.frame ready to be analyzed. Convert, change to NAs, etc
  n_n <- data.frame(ID = pma$id,
                    first = pma$first, middle = pma$middle, last = pma$last,
                    groupID = NA,
                    address = pma$address, email = pma$email, 
 ...
 unique_em <- n_n$email[!is.na(n_n$email) & is.na(n_n$groupID)]
  ...
  for (p in unique_groupid) {
    ...
    if (!anyNA(n_n1$ID)) {
      ...
      match2 <- !is.na(name_df$m_i) &
        n_n1$m_i == name_df$m_i &
        n_n1$f_c > 1 & name_df$f_c > 1
      # match by address
      match3 <- !is.na(name_df$address) & n_n1$address == name_df$address
      if (sum(ifelse(is.na(c(match1, match2, match3)), FALSE,
                     c(match1, match2, match3))) > 0) {
        matched <- TRUE
    ...
    if (!matched & nrow(n_n1) > 0 & !anyNA(n_n1$ID)) {
      ...
      nninf <- apply(n_n1[choice, c("address", "email")],
  ...
  for (q in unique_names_over2) {
    ...
    confidence <- sc1 + sc2 + sc3 + sc4 + sc7 + sc8 + sc9 + sc10
  ...
  test_group$nid <- test_group$ID
  
  test_group$similarity2 <- test_group$similarity
  
  test_group$similarity2[is.na(test_group$similarity2)]<-1 
  
  test_group <- test_group %>% mutate(nid=ifelse(similarity2<.6, nid, groupID))
  
  test_group <- test_group %>% add_count(nid)
  
  tg2 <- test_group[with(test_group, order(-n, nid)), ]

  #save(tg2, file="data_contexts2/names_disambig.Rda")
  
  #after blend back into author by article file.
  
  tgmer <- tg2 %>% select(ID, nid)
  
  pma$id <- as.numeric(row.names(pma))
  
  pma2 <- left_join(pma, tgmer, by=c("id"="ID"))
  
  pma2 <- pma2 %>% group_by(nid) %>% add_count(pmid)
  
  
  #a small handful of teams (~1,600) assigned one author id...let's collapse those to one id
  
pma2 <- pma2[!duplicated(t(apply(pma2[c("pmid", "nid")], 1, sort))), ]

load("data/pubmed_articles2.Rda")

article_cut <- article_df %>% select(pmid, year, month)
  
pma2 <- left_join(pma2, article_cut)

pma2 <- pma2 %>% filter(month!="07", year>2019)

#This is key file moving forward

save(pma2, file="data/author_long2.Rda")
